...
namespace Foodiefeed.extension
{
    public static class ColorFadeAnimationExtension
    {
            ...
            public static void CancelAnimation(this VisualElement element, string handle)
            {
            ...
            private static Task<bool> ColorAnimation(VisualElement element, string name, Func<double, Color> transform, Action<Color> callback, uint length, Easing easing)
            {
                ...
                new Animation(v => callback(transform(v)))
                    .Commit(element, name, 16, length, easing, (v, c) => taskCompletionSource.SetResult(c));
